...
class BPlusTree {
 public:
  ...
  BPlusTree(unsigned N, unsigned M)
      : depth(0), N(N), M(M), root(new_leaf_node(M)) {
  ...
  void insert(KEY key, VALUE value) {
    ...
    if (depth == 0) {
      ...
      was_split =
          leaf_insert(reinterpret_cast<LeafNode*>(root), key, value, &result);
    } else {
      // The root is an inner node
      was_split = inner_insert(reinterpret_cast<InnerNode*>(root), depth, key,
                               value, &result);
    }
    if (was_split) {
  ...
  void getRange(const KEY& key1, const KEY& key2,
                std::multimap<KEY, VALUE>& res) {
    InnerNode* inner;
    ...
    if (depth == 0) {
      ...
      while (index < leaf->num_keys) {
        if (leaf->keys[index] >= key1 && leaf->keys[index] <= key2) {
          res.insert(
              std::pair<KEY, VALUE>(leaf->keys[index], leaf->values[index]));
        }
        ++index;
      }
    }
  ...
  // Recursively traverse leaf nodes within the range of (key1, key2)
  void windowQuery(void* node, int depth, const KEY& key1, const KEY& key2,
                   std::multimap<KEY, VALUE>& res) {
    InnerNode* inner;
 ...
 private:
  ...
  bool leaf_insert(LeafNode* node, KEY& key, VALUE& value,
                   InsertionResult* result) {
  ...
  bool inner_insert(InnerNode* node, unsigned depth, KEY& key, VALUE& value,
                    InsertionResult* result) {
    assert(depth != 0);
  ...
  void inner_insert_nonfull(InnerNode* node, unsigned depth, KEY& key,
    ...
    if (depth - 1 == 0) {
      // The children are leaf nodes
      was_split =
          leaf_insert(reinterpret_cast<LeafNode*>(node->children[index]), key,
                      value, &result);
    } else {
